...
// build/dev/javascript/dnd/dnd/web.mjs
...
function get_id(pair) {
  return pair[0];
}
function get_content(pair) {
  return pair[1];
}
function delete_card(column2, card2) {
  let content = (() => {
    let _pipe = column2;
    return get_content(_pipe);
  })();
  return [
    (() => {
      let _pipe = column2;
...
function append_card(column2, card2) {
  let content = (() => {
    let _pipe = column2;
    return get_content(_pipe);
  })();
  return [
    ...
    (() => {
      ...
      return new Column(
        _record.title,
        append2(content.cards, toList([card2]))
      );
    })()
  ];
}
function move_card(columns, from, to, card2) {
  let $ = find2(
    columns,
    (pair_column) => {
      return (() => {
        let _pipe = pair_column;
        return get_id(_pipe);
      })() === from;
    }
  );
  ...
  return map(
    ...
    (pair_column) => {
      ...
      if (id2 === from) {
        let _pipe = from_column;
        return delete_card(_pipe, card2);
      } else if (id2 === to) {
        let _pipe = to_column;
        return append_card(_pipe, card2);
      } else {
        return pair_column;
      }
    }
...
// build/dev/javascript/dnd/dnd/message.mjs
...
// build/dev/javascript/dnd/dnd/event.mjs
...
// build/dev/javascript/dnd/dnd.mjs
...
function init2(_) {
  return new Model2(
    toList([
      [1, new Column("A", toList([[3, new Card("A1")], [4, new Card("A2")]]))],
      [2, new Column("B", toList([[5, new Card("B1")], [6, new Card("B2")]]))]
    ]),
    new None(),
    new None(),
    new None()
  ...
  return div(
    ...
    append2(
      ...
      map(
        column2[1].cards,
        (pair_card) => {
          return card(pair_card, column2[0]);
        }
      )
    )
  );
